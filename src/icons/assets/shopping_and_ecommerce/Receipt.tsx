import React from 'react'
import { IconProps } from '@/icons/IconProps'

export const OutlineReceipt: React.FC<IconProps> = ({ className, size }) => (
   <svg
      width={size}
      height={size}
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      className={className}>
      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M4 3h16v15.492c0 2.056-2.345 3.233-3.994 2.005l-.704-.525a.5.5 0 0 0-.599 0l-.303.228a4 4 0 0 1-4.8 0l-.303-.227a.5.5 0 0 0-.599-.001l-.704.525C6.344 21.725 4 20.548 4 18.492V3Zm2 2v13.492a.5.5 0 0 0 .799.4l.704-.524a2.5 2.5 0 0 1 2.994.005l.303.227a2 2 0 0 0 2.4 0l.303-.227a2.5 2.5 0 0 1 2.994-.005l.704.525a.5.5 0 0 0 .799-.401V5H6Z'
         fill='currentColor'
      />

      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M9 8a1 1 0 0 0 0 2h2a1 1 0 1 0 0-2H9Zm6 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2Zm-7 5a1 1 0 0 1 1-1h2a1 1 0 1 1 0 2H9a1 1 0 0 1-1-1Zm7-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z'
         fill='currentColor'
      />

      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M3 4a1 1 0 0 1 1-1h16a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1Z'
         fill='currentColor'
      />
   </svg>
)

export const SolidReceipt: React.FC<IconProps> = ({ className, size }) => (
   <svg
      width={size}
      height={size}
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      className={className}>
      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M5 8a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v12.5a.5.5 0 0 1-.692.462l-1.988-.829a1 1 0 0 0-.654-.04l-2.857.8a2.999 2.999 0 0 1-1.618 0l-2.857-.8a1 1 0 0 0-.654.04l-1.988.829A.5.5 0 0 1 5 20.5V8Zm3 3a1 1 0 0 1 1-1h2a1 1 0 1 1 0 2H9a1 1 0 0 1-1-1Zm7-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2Zm-7 5a1 1 0 0 1 1-1h2a1 1 0 1 1 0 2H9a1 1 0 0 1-1-1Zm7-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z'
         fill='currentColor'
      />

      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M3 4a1 1 0 0 1 1-1h16a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1Z'
         fill='currentColor'
      />
   </svg>
)

export const DuotoneReceipt: React.FC<IconProps> = ({ className, size }) => (
   <svg
      width={size}
      height={size}
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      className={className}>
      <path
         fillRule='evenodd'
         clipRule='evenodd'
         className='opacity-50'
         d='M5 4h14v16.5a.5.5 0 0 1-.692.462l-1.988-.829a1 1 0 0 0-.654-.04l-2.857.8a2.999 2.999 0 0 1-1.618 0l-2.857-.8a1 1 0 0 0-.654.04l-1.988.829A.5.5 0 0 1 5 20.5V4Z'
         fill='currentColor'
      />

      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M9 9a1 1 0 0 0 0 2h2a1 1 0 1 0 0-2H9Zm6 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2Zm-7 5a1 1 0 0 1 1-1h2a1 1 0 1 1 0 2H9a1 1 0 0 1-1-1Zm7-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z'
         fill='currentColor'
      />

      <path
         fillRule='evenodd'
         clipRule='evenodd'
         d='M3 4a1 1 0 0 1 1-1h16a1 1 0 1 1 0 2H4a1 1 0 0 1-1-1Z'
         fill='currentColor'
      />
   </svg>
)
